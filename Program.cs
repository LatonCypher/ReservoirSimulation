{
    ...
    var Producer = (MinPressure: 1000.0, ProdRate: -500.0, OilRate: 0.0, WaterRate: 0.0, Index: 0);
    var Injector = (MaxPressure: 5000.0, InjRate: 500.0, OilRate: 0.0, WaterRate: 0.0, Index: 24);

    // Post Processing
    Subplot(7, 4, [0, 1, 4, 5]);
    var Pbhp = Plot([0], [0], "r", 2);
    Axis([0, 100000, 0, Injector.MaxPressure*1.1]);
    Title("Producer BHP");

    Subplot(7, 4, [8, 9, 12, 13]);
    var Prate = Plot([0], [0], "r", 2);
    Axis([0, 100000, 0, Producer.ProdRate*1.1]);
    Title("Producer Rate");
    ...
    Axis([0, 100000, 0, Injector.InjRate*1.1]);
    Title("Injector Rate");
    ...
    double[] Residual(double[] xnp1)
    {
        ...
        double[] Rw = Zeros(Nblocks), Ro = Zeros(Nblocks),
            Rwells = Zeros(Nwells), Rdecision = Zeros(Nwells);
        ...
        Rdecision[0] = RateControl[0] ? Qwells_np1[0] - Producer.ProdRate : Pwells_np1[0] - Producer.MinPressure;
        ...
        Rdecision[1] = RateControl[1] ? Qwells_np1[1] - Injector.InjRate : Pwells_np1[1] - Injector.MaxPressure;

        return Pack(Ro, Rw, Rwells, Rdecision);
    }

    double[] xs = Pack(Po_n, Sw_n, Pwells_n, Qwells_n), xn;
    var opts = SolverSet(Display: true, MaxIter: 10, AbsTol: 1e-6, UseParallel: true);
    while (Time.Last() < 120000)
    {
    ...
    int Skip = Max(1, Time.Count/500), indx;
    // Post Processing
    byte[] Animfun(int i)
    {
        indx = i*Skip;
        Pbhp.Xdata = Time[..indx]; Pbhp.Ydata = ProdPwf[..indx];
        Prate.Xdata = Time[..indx]; Prate.Ydata = ProdRate[..indx];
        Pbsw.Xdata = Time[..indx]; Pbsw.Ydata = WaterCut[..indx];

        Ibhp.Xdata = Time[..indx]; Ibhp.Ydata = InjPwf[..indx];
        Irate.Xdata = Time[..indx]; Irate.Ydata = InjRate[..indx];
        Iswp.Xdata = Time[..indx]; Iswp.Ydata = SweepEff[..indx];
    ...
    AnimationMaker(Animfun, "WaterFlooding.gif", 30, Time.Count/Skip);
    CloseFig();
}
